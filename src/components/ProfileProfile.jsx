...
export default function ProfileProfile() {
  ...
  return (
    <div className="grid grid-cols-4  p-4 md:grid-cols-2 lg:grid-cols-3 gap-6  mx-auto rounded-lg">
        ...
        {leads.map((lead) => (
          <div className="grid grid-cols-3  gap-x-4 p-2" key={lead.id}>
            ...
            <div className="m-2">
              ...
              <p>Ksh {Number(lead.amount) - Number(lead.amount_paid || 0)}</p>
            </div>
            <div className="m-2">
              <p className="font-medium">Assigned Agent:</p>
              <p>{lead.agent_name || "Unassigned"}</p>
      ...
      {/* PTP & Updates */}
      <UploadProof leadId={leadId} refreshLead={() => {
        fetchLeads();            // refresh lead data
        handleRefreshPayments(); // 👈 trigger payment refresh
      }} />

      <PaymentHistory leadId={leadId} refreshKey={refreshPayments}/>
